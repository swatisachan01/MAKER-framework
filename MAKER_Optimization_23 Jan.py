...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 21 21:22:46 2019

@author: swatisachan
"""
#Last attribute or varibable in dataset is assumed to output variable and other are input variable
...
Data_file = r'/Users/swatisachan/Desktop/Computers AND OR/Asthma_COR.xlsx'
xls_file = pd.ExcelFile(Data_file)
print(xls_file.sheet_names)
data1 = xls_file.parse('RE_PE') ##sheet to import
...
idx = data1.iloc[:,-1].unique() #Put UNKNOWN laways in first in the list
if 'UNKNOWN' in idx:
...
def MAKER_func1():
    ...
    #2)Interrelation from complete data    
    #Denominator
    ...
    for i in range(start,end):
        ...
        if i == start:
            d1,d2 = Prob_df[0],Prob_df[1]
        else:  
            nn = Prob_df[i-1].copy() #copy joint probability p[i-1] to nn dataframe, so that multilevel structure of prob_df does not change                                       
            nn.columns = nn.columns.map('|'.join) #merge columns of joint probability
            d1,d2 = nn,Prob_df[i-s1] #multiple joint with single. First joint p[i-1]*single[i-s1] 
    ...
    for i in range(0,len(Multi_deno)):
        ...
        temp_inter = Prob_df[start1+i].values/Multi_deno[i].values #division
        temp_inter = np.nan_to_num(temp_inter) #replace nan with 0        
        temp_inter = pd.DataFrame(temp_inter,columns=Prob_df[start1+i].columns, index=Prob_df[start1+i].index) #add multilevel and index in dataframe
...
def objective_func():
    ...
    ######long process#####
    #df1 = (Observed - Estimated)**2
    #print(df1)
    #sum_row = df1.sum(axis=1)
    #print(sum_row)
    #sum_col = sum_row.sum(axis = 0)
    #print(sum_col)
    #error = sum_col/(2*len(Estimated))
    #print(error)
...
solution = minimize(optimization,initial_parameter,method='SLSQP',bounds=constrain_bound,options={"disp": True,'eps' : 3e-1,'maxiter':30}) 
x = solution.x
